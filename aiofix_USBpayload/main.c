...
void* load_elf_segments(const uint8_t* data, size_t data_size) {
...
int load_from_data(const uint8_t* data, size_t data_size) {
    if (!data || data_size == 0) {
        return -1;
    }
...
void run_usb_payload_logic() {
    ...
    // Priority 3: load integrated one
    load_from_data(henData, sizeof(henData));
    if (run_payload() == 0) {
        wait_for_payload_to_exit();
    }
